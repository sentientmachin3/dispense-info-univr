...
\begin{document}
	...
	I principali sono:
	\begin{itemize}
		\item Singleton: è usato per assicurare che una classe abbia una sola istanza per tutto il ciclo di vita del software. Le classi Singleton hanno costruttori privati;
		
		\item Observer: è usato per definire una dipendenza uno-a-molti tale che quando un oggetto cambia il suo stato tutti gli altri oggetti vengono notificati e aggiornati di conseguenza;
		
		\item Data Transfer Object: è usato per trasferire dati tra un client e un server;
		
		\item Adapter: è usato per convertire un interfaccia di una classe in un'altra richiesta dal client, consentendo a diverse classi di operare insieme quando altrimenti ciò non sarebbe possibile;
		
		\item Template: è usato per definire la struttura di un algoritmo in una superclasse delegando alcuni passi a una o più sottoclassi;
		...
		\item Proxy: una classe proxy crea un oggetto e lo tiene salvato per evitare di instanziarlo nuovamente per utilizzi futuri;
