...
CostruisciIndSet($ G $, $ k^* $)
	...
	\subsection{Problema SAT-Search}
		...
		\begin{lstlisting}[frame = tb, mathescape=true, caption={Algoritmo di Ricerca per SAT}]
SAT-Solver($ \phi $)
	if $ \alg{A}(\phi) = no $:
		return $ no $
	for $ i = 1 $ to $ n $:
	...
	\subsection{Self Reducible}
		...
		\begin{proof}
			...
			\end{align*}
			Possiamo produrre $ w $ usando l'algoritmo SAT-solver. La risposta di tale algoritmo sarà uguale alla risposta dell'oracolo
			\[
