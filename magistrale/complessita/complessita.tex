...
SAT($ \phi(x_1, \dots, x_n) $)
	...
	return $ no $
		...
		\begin{proof}
			...
			\begin{lstlisting}[mathescape=true, frame=tb, caption={Algoritmo ricorsivo per risolvere Reachability}]
MiddleSearch($ G, s, t,$ length)
	if length $ \leq 1 $:
		if $ s = t $ or $ (s, t)\in E $:
			return $ yes $
		else:
		...
		foreach $ u \in V $:
			if (MiddleSearch($ G, s, u, \dfrac{length}{2} $) $ = yes $ and 
				MiddleSearch($ G, u, t, \dfrac{length}{2} $) $ = yes $):
