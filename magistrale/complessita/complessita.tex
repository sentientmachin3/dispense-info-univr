...
SAT-Solver-ND($ \phi(x_1, \dots, x_n) $) "\hspace{.8cm}\tikzmark{zero}"
	...
	\section{Alcuni problemi NP-Completi}
		...
		\begin{proof}
			...
			\begin{enumerate}
				...
				\begin{figure}[h!]
					...
					\draw (g) -- (h) -- (i) -- (g);\draw (l) -- (m) -- (n) -- (l);
					\draw (g) -- (b);\draw (h) -- (c);\draw[bend right=90] (i) to (f);\draw (l) -- (a);
					\draw[bend right=90] (m) to (d);\draw (n) -- (f);
					\end{tikzpicture}
				\end{figure}
				...
				\begin{figure}[h!]
					...
					\begin{tikzpicture}
						...
						\draw (g) -- (b);\draw (h) -- node[midway] {|} (c);\draw[bend right=80] (i) to node[midway] {|} (f);\draw (l) -- (a);
		...
		\subsection{Classe di problemi DP e Problema Clique-No-Clique}
			\begin{definit}[classe di problemi $ \DP $]
				\[
					\DP = \big\lbrace \prob{A}\ \Big|\ \exists \prob{B},\prob{C}\in \np\quad \instance{A} = \instance{B} = \instance{C} \ t.c.\ \prob{A}(x) = yes\ \Leftrightarrow\ \prob{B}(x) = yes\ \wedge\ \prob{C}(x) = yes \big\rbrace
				\]
			\end{definit}
	...
	\section{Complessità di Spazio e la classe SPACE}
		...
		\begin{definit}[Classe \textbf{SPACE}]
			...
			\begin{align*}
				\mathbf{SPACE}(f(n)) = \big\lbrace & \prob{A}\ \Big|\ \text{ esiste un programma/algoritmo che risolve istanze di } \prob{A} \\ 
