...
VerifyK-Colouring(G = (V,E), $ f(v_1), \dots, f(v_n) $)
	...
	\section{Classe di problemi NP-Completi}
		...
		\noindent
		\textbf{\textit{NP-Completo}}$ = \{ \exists p(x) = x^k, \exists V(. , .) t.c. T_V(a, b) = \mathcal{O}(p(|a|+|b|)) e \forall x \in \mathcal{I}(\mathbb{A}), \mathbb{A}(x)=yes \Leftrightarrow \exists w \in \{0, 1 \}^{p(|x|)}, V(x, w)=yes \}$\\
...
MiddleSearch($\medmath{ G, s, t,}$ length)
	if length $ \medmath{\leq 1} $:
		...
		$\nspace (f(n))$ = $\{ \mathbb{A} | \exists V(. ; .)$ deterministico tale che $\forall x \in \mathcal{I}(\mathbb{A}) \mathcal{A}(x)= yes$ sse $V(x, w)= yes$ e $V$ usa al più $\mathcal{O}(f(n))$ di memoria di lavoro (escludendo $x$ e $w$), $V$ accede a $x$ in maniera Read-Only, $V$ accede a $w$ in maniera Read-Only e Left-to-Right $\}$
	...
	\begin{lstlisting}[mathescape=true, frame=tb, caption={SAT-Solver-ND $\in \npspace(n)$}, basicstyle=\footnotesize\ttfamily]
		...
		\newline
		\noindent
		$\pspace = \npspace = \{ \mathbb{A} | \exists V(. ; .)$ deterministico tale che $\forall x \in \mathcal{I}(\mathbb{A}) \mathcal{A}(x)= yes$ sse $V(x, w)= yes$ e $V$ usa al più $\mathcal{O}(|x|^k)$ di memoria di lavoro (escludendo $x$ e $w$), $V$ accede a $x$ in maniera Read-Only, $V$ accede a $w$ in maniera Read-Only e Left-to-Right $\}$
	...
	\section{Approssimazione}
		...
		\begin{definit}
			Sia $\mathbb{A}$ un problema di \textbf{minimizzazione}, $\mathcal{A}$ è un algoritmo di \textit{k-approssimazione} per $\mathbb{A}$ se: $~~\forall x \in \mathcal{I}(\mathbb{A}) ~~\frac{VAL(\mathcal{A}(x))}{VAL(OPT(x))}\leq k$
		\end{definit}
	
	\begin{definit}
		Sia $\mathbb{A}$ un problema di \textbf{massimizzazione}, $\mathcal{A}$ è un algoritmo di \textit{k-approssimazione} per $\mathbb{A}$ se: $~~\forall x \in \mathcal{I}(\mathbb{A}) ~~\frac{VAL(OPT(x))}{VAL(\mathcal{A}(x))}\leq k$
	\end{definit}

\noindent
\textit{OPT(x)}: è una soluzione di valore massimo/minimo per l'istanza $x$.\\
...
Un algoritmo di \textit{1-approssimazione} è un algoritmo ottimo.
	...
	Il problema di decisione \textit{Partition} è $\npc$ e sappiamo che \textit{Partition} $\leq$ \textit{Makespan} (con $m=2$).
		...
		\textit{\textbf{PTAS}} è un algoritmo che data $x\in \mathcal{I}(\mathbb{A})$ e $\epsilon$, fornisce una soluzione $s$ per $x$ t.c. $\frac{VAL(\mathcal{A}(x))}{VAL(OPT(x))}~\leq~(1+~\epsilon).$\\
		Il running-time del \textit{PTAS} $\mathcal{A}$ è $\mathcal{O}(|x|^k)$ (polinomiale in $|x|$ ma può essere esponenziale in $(\frac{1}{\epsilon})$).\\
		\\
		\textit{\textbf{FPTAS}} è un algoritmo che data $x\in \mathcal{I}(\mathbb{A})$ e $\epsilon$, fornisce una soluzione $s$ per $x$ t.c. $\frac{VAL(\mathcal{A}(x))}{VAL(OPT(x))}~\leq~(1+~\epsilon).$\\
		...
		\noindent
		Se esiste un problema $\mathbb{B} \in \npc$ t.c. $\mathbb{B} \leq (a,b)-\mathbb{A}$, allora non esiste \textit{non esiste} un algoritmo di approssimazione polinomiale per $\mathbb{A}$ che garantisce approssimazione $k < \frac{b}{a}$ se P$\neq$ NP.
		...
		\begin{itemize}
			...
			\item Output: seegnamento che soddisfa il massimo numero di clausole.
		\end{itemize}
